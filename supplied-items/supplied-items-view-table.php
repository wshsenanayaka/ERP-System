  ...
  {
      $search =$_POST["query"];
      $query_r_view = "SELECT * FROM suppliedreportinfor WHERE title LIKE '%".$search."%' ";
  }
  else
  {
      $query_r_view ="SELECT * FROM suppliedreportinfor";
  }
  ...
  <table id="editable_table" class="table table-sm">
       ...
       while($row = mysqli_fetch_array($result_r_view))
       {
        ...
        <tr>
          ...
          <td><a href="supplied-items-view1?sview_id='.$row["id"].'&update=true">View</a></td>
         </tr>';
